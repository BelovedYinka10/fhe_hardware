...
class Gate:
    ...
    def compute_poly(self, data : list):
        if self.right == -1:
            if self.left == -1:
                return data[0] - data[0]  # zero element (padding gate)
            return data[self.left].copy() if hasattr(data[self.left], 'copy') else data[self.left]
    ...
    def compute_int(self, mod : int, data : list[int]) -> int:
        if self.right == -1:
            return 0 if self.left == -1 else data[self.left]
        if len(data) < self.right:
            raise Exception("data list is small")
        if self.op == OpType.ADD:
    ...
    def compute_list(self, data : list[list[Field]]) -> list[Field]:
        if self.right == -1:
            if self.left == -1:
                return [data[0][i] - data[0][i] for i in range(len(data[0]))]
            return list(data[self.left])
        if len(data) < self.right:
            raise Exception("data list is small")
        ret = []
        if self.op == OpType.ADD:
