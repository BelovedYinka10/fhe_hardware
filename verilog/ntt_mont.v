...
module ntt_mont #(
    ...
    input  wire [Q_WIDTH-1:0]    n_inv,    // N^{-1}·R mod q (Montgomery form)
    input  wire [Q_WIDTH-1:0]    q_inv,    // -q^{-1} mod 2^Q_WIDTH  (R = 2^Q_WIDTH)
    ...
    // mod_mul : Montgomery multiply — returns a·b·R^{-1} mod q, R = 2^Q_WIDTH
    //   q_inv [Q_WIDTH-1:0] = -q^{-1} mod R, read from the module input port.
    //
    //   Callers pass the *constant* operand pre-scaled into Montgomery form
    //   (b' = b·R mod q), so mont(a, b') = a·b·R·R^{-1} = a·b mod q — i.e. a
    //   drop-in replacement for the Barrett mod_mul at every call site. Only
    //   the twiddle table and n_inv need pre-scaling by R; coefficients stay
    //   in the normal domain.
    //
    //   Widest multiply here is Q_WIDTH × Q_WIDTH. The Barrett version needed
    //   p·M at 2·Q_WIDTH × 2·Q_WIDTH (120×120 at Q_WIDTH=60), which is what
    //   made it expensive.
    //
    //   T  [2·Q_WIDTH-1 : 0]  — product a·b            (< q^2)
    //   m  [Q_WIDTH-1   : 0]  — (T mod R)·q_inv mod R
    //   mq [2·Q_WIDTH-1 : 0]  — m·q
    //   s  [2·Q_WIDTH   : 0]  — T + m·q, divisible by R by construction
    //   t  [Q_WIDTH     : 0]  — s >> Q_WIDTH ∈ [0, 2q), one correction
    function automatic [Q_WIDTH-1:0] mod_mul;
        input [Q_WIDTH-1:0] a, b, qq;
        reg [2*Q_WIDTH-1:0] T;
        reg [Q_WIDTH-1:0]   m;
        reg [2*Q_WIDTH-1:0] mq;
        reg [2*Q_WIDTH:0]   s;
        reg [Q_WIDTH:0]     t;
        begin
            T  = a * b;
            m  = T[Q_WIDTH-1:0] * q_inv;          // low Q_WIDTH bits only
            mq = m * qq;
            s  = {1'b0, T} + {1'b0, mq};
            t  = s[2*Q_WIDTH : Q_WIDTH];          // exact divide by R
            if (t >= {1'b0, qq})
                t = t - {1'b0, qq};
            mod_mul = t[Q_WIDTH-1:0];
        end
    endfunction
